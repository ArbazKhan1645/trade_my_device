...
class OtherSectionBody extends StatefulWidget {
  const OtherSectionBody({super.key});

  @override
  State<OtherSectionBody> createState() => _OtherSectionBodyState();
}

class _OtherSectionBodyState extends State<OtherSectionBody> {
  ...
  void _fetchListArguments() {
    final String? category = Get.arguments?['option'] as String?;
    if (category == null) {
      selectedCategory = '';
      return;
    }
              ...
              RichText(
                text: TextSpan(
                  text: selectedCategory == ''
                      ? 'Others'
                      : categories[selectedCategory]?[0]['title'] ?? 'Others',
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            return selectedCategory == ''
                ? _buildCategoryItem(item as String)
                : _buildSubCategoryItem(item as Map<String, dynamic>);
          },
        ),
      ),
    );
  ...
  Widget _buildSubCategoryItem(Map<String, dynamic> item) {
    return InkWell(
      child: Card(
        elevation: 0,
        margin: const EdgeInsets.all(16.0),
  ...
  double _calculatePadding(double width) {
    if (width >= 1200) return 250.0; // Large screen
    if (width >= 800) return 150.0; // Medium screen
